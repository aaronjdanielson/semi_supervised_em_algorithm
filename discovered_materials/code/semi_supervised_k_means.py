...
## currently refactoring this file for readability

def cop_kmeans(dataset, k, must_link=[], cannot_link=[],
               initialization='kmpp',
               max_iter=300, tol=1e-4):

    must_link, cannot_link = transitive_closure(must_link, cannot_link, len(dataset))
    must_link_info = get_must_link_info(must_link, dataset)
    ...
    for _ in range(max_iter):
        ...
        for i, d in enumerate(dataset):
            ...
            if clusters_[i] == -1:
                ...
                while (not found_cluster) and counter < len(indices):
                    ...
                    if not violate_constraints(i, index, clusters_, must_link, cannot_link):
                        found_cluster = True
                        clusters_[i] = index
        ...
        clusters_, centers_ = compute_centers(clusters_, dataset, k, must_link_info)
...
def get_must_link_info(ml, dataset):
